...
test('manual action placement, learning, future matching, undo and stale menu protection', async () => {
  ...
  try {
    ...
    manual.click();
    assert.equal(article.getAttribute('data-xjev-folded'), 'true');
    assert.equal(manual.hasAttribute('aria-haspopup'), false);
    assert.equal(writes, 1, 'complete rules save immediately');
    assert.equal(w.document.querySelector('.xjev-rule-editor'), null);
    await sleep(300);
    assert.equal(article.getAttribute('data-xjev-folded'), 'true');
    ...
    assert.equal(w.document.querySelector('#xjev-toast, .xjev-menu, .xjev-rule-editor'), null);
    article.querySelector('.xjev-placeholder button:last-child').click();
    await sleep(300);
    assert.equal(w.document.querySelectorAll('[data-xjev-folded]').length, 0);
    const staleAction = article.querySelector('.xjev-manual');
    article.querySelector('[data-testid="tweetText"]').textContent = 'Recycled comment';
    staleAction.click();
    assert.equal(writes, 1);
    await sleep(300);
    article.querySelector('[data-testid="tweetText"]').textContent = '好呀';
    await sleep(300);
    article.querySelector('.xjev-manual').click();
    assert.equal(article.getAttribute('data-xjev-folded'), 'true', 'collapse immediately');
    await sleep(300);
    assert.equal(writes, 2);
    assert.equal(cfg.rules[0].match, 'post', 'generic short text must not become a broad keyword');
    assert.equal(w.document.querySelector('#xjev-toast, .xjev-menu, .xjev-rule-editor'), null);
    w.document.body.insertAdjacentHTML('beforeend', post('323', '好呀', actions));
    await sleep(300);
    assert.equal(w.document.querySelectorAll('[data-xjev-folded]').length, 1, 'unrelated short replies remain visible');
