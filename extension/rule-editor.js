(() => {
  function open(draft, { source, onSave, onCancel = () => {} } = {}) {
    ...
    const title = document.createElement('h2'); title.textContent = draft.recordId ? '编辑屏蔽规则' : '预览并保存规则'; form.append(title);
    const hint = document.createElement('p'); hint.textContent = '只有保存后的规则才会用于后续信息流。草稿来自本地特征提取，请检查是否过于宽泛。保存时会在本机保留来源样本。'; form.append(hint);
    const fields = {};
    ...
    field('keywords', '关键词 · 每行一个', { multiline: true, help: '按包含文字匹配，不区分英文大小写，不使用正则表达式。' });
    ...
    field('description', '要屏蔽的具体内容模式', { multiline: true, max: 2000, help: '例如：推广交易平台返佣，并引导注册或私信。由你明确规则，Jev 判断是否命中；需要开启 Jev 总开关与语义规则开关。' });
    ...
    if (source) {
      const details = document.createElement('details'); const summary = document.createElement('summary'); summary.textContent = '查看来源样本与提取特征';
      ...
      details.append(summary, text, detail); form.append(details);
    } else if (draft.sourceId) { const note = document.createElement('p'); note.textContent = '来源样本已删除或淘汰；保存的规则仍然有效。'; form.append(note); }
    ...
    form.onsubmit = async event => {
      ...
      } catch (error) { status.textContent = error.message || '保存失败，请重试'; save.disabled = false; cancel.disabled = false; }
    };
